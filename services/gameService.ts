...
class GameService {
  ...
  public emitPlaceUnit(payload: PlacePayload) {
  ...
  public attackUnit(attackerId: string, targetId: string) {
    if (this.state.appStatus !== AppStatus.PLAYING) return;
    if (this.checkPlayerRestricted(this.state.currentTurn)) return;
  ...
  public confirmMove() {
    ...
    if (unitIndex === -1 || units[unitIndex].playerId !== currentTurn) return;
  ...
  public skipTurn() {
      if (this.state.appStatus !== AppStatus.PLAYING) return;
      if (this.state.interactionState.mode !== 'NORMAL' || this.state.selectedCardId || this.state.selectedUnitId) { this.cancelInteraction(); return; }
      this.log(`> TURN ENDED BY USER`, this.state.currentTurn);
