...
const CardCatalogue: React.FC<CardCatalogueProps> = ({ onClose }) => {
    ...
    const renderGridItem = (type: string, isAbility: boolean = false, isTalent: boolean = false) => {
        let name = type;
        ...
        const iconColor = isSelected ? '#fff' : (isUpgrade ? (talent?.color || '#60a5fa') : (isModule ? (ability?.color || '#fbbf24') : (isAction ? '#a855f7' : COLORS.P1)));
        ...
        return (
            ...
            >
                ...
                <div className="text-center w-full">
                    <div className={`text-[8px] font-bold uppercase truncate ${isSelected ? 'text-white' : (isUpgrade ? 'text-blue-200' : (isModule ? 'text-yellow-200' : (isAction ? 'text-purple-300' : 'text-gray-400')))}`}>
                        {name}
                    </div>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="flex w-[90vw] max-w-5xl h-[80vh] border border-cyan-500/30 bg-black/80 rounded-2xl shadow-[0_0_50px_rgba(0,255,255,0.1)] overflow-hidden backdrop-blur-xl">
                ...
                <div className="w-80 flex flex-col bg-black/40 backdrop-blur-md relative">
                    ...
                    {!selectedConfig && !selectedAbility && !selectedTalent && (
